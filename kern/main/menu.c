 ...
 */
//------------------------------------------------------------------------------------------------
static const char *opsdebugmenu[] = {
...
//---------------------------------------------------------------------------------------------------
static int cmd_df1(int nargs, char **args)
{	 
    int a = atoi(args[1]);
    char* status = args[2];
    char* onmsg = "on";
	...
	if (a <= 12 && a >= 1)
	{  
		if (strcmp(status, offmsg))
		{	
			bin[12 - a] = '1';
		}
		else if (strcmp(status, onmsg))
		{
			bin[12 - a] = '0';
		}
		else 
...
cmd_dbflags(int nargs, char **args)
{
	...
	showmenu("OS/161 Debug flags", opsdebugmenu);
	kprintf("\n");
