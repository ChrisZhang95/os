...
/*Lock for the pid data structure*/
static struct lock *plock;

/*Data Structure to store all the pid*/
static struct process_info* pid_all[MAX_PROCESS];
	    ...
	    struct thread **ret)
{
	...
	/*Give child process a pid*/
	lock_acquire(plock);
	newguy->pid = newPid(curthread->pid, &result);
	lock_release(plock);
...
int newPid(pid_t parentPid, int32_t *retval){
	int index;
	for(index = 0; index < MAX_PROCESS; index++){
		if(pid_all[index] == NULL) break;
	}
	pid_all[index] = pid_allocator(index, parentPid);
	return index; 
}

struct process_info* pid_allocator(pid_t self_pid, pid_t parent_pid){

	struct process_info* temp = kmalloc(sizeof(struct process_info));
	
	temp->self_pid = self_pid;
	...
	if(temp->out_sema == NULL){
		temp->out_sema = sem_create("out_sema", 0);
	}
	return temp; 
...
void pid_bootstrap(void){
	pid_all[0] = pid_allocator(0, NULL);
	
        plock = NULL;
        
        if (plock == NULL) {
            //initialize the pid lock;
            plock = lock_create("plock");
	
	}
	
}

void pid_destroy(pid_t target){
	sem_destroy(pid_all[target]->out_sema);
	kfree(pid_all[target]);
	pid_all[target] = NULL;
	return;
}
...
void pid_exit(int exitcode){
	int index;
	for (index = 0; index < MAX_PROCESS; index++){
		if(pid_all[index]->self_pid == curthread->pid)
		break;
	}
		
	if(pid_all[index]->num_waiter != 0){
		pid_all[index]->exit_status = exitcode;
		pid_all[index]->exit = 1;
		V(pid_all[index]->out_sema);
		return;
	}
	else{
		pid_destroy(index);
...
int thread_wait(pid_t target, int* set_status, int* retval){
	if(pid_all[target] == NULL){
		*set_status = 0;
		*retval = target;
		return 0;
	}
	
	if(pid_all[target]->parent_pid == curthread->pid){
		pid_all[target]->num_waiter++;
		P(pid_all[target]->out_sema);
		*set_status = pid_all[target]->exit_status;
		*retval = target;		
		pid_all[target]->num_waiter--;
		
		if(pid_all[target]->num_waiter != 0){
			V(pid_all[target]->out_sema);
